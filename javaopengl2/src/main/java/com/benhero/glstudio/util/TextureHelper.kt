...
object TextureHelper {
    ...
    /**
     * 根据 资源ID 获取相应的 OpenGL 纹理ID，若加载失败则返回0
     * <br></br>必须在GL线程中调用
     */
    fun loadTexture(context: Context, resourceId: Int): TextureBean {
        ...
        // 2. 将 纹理 绑定到 OpenGL 对象上
        // 纹理目标 : GL_TEXTURE_2D ，纹理ID : textureObjectIds[0]
        // 将纹理ID与纹理目标绑定
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textureObjectIds[0])

        // 不知道这里怎么找到对应的纹理，给他设置属性的，官方解释是因为第二步的操作，把纹理ID与纹理目标绑定后，通过纹理目标，就可以找到对应的纹理对象？
        // 还是说通过相邻的方式
        ...
        // 4. 通过 OpenGL对 象读取 Bitmap数据，并且 绑定到 纹理对象上，之后就可以回收Bitmap对象
